...
sub _fake_complete_splice_success ($self, $token, $bytes) {
    my ($rec, $op) = $self->_take_pending_and_unregister('pending_splices', $token) or return 0;

    $op->_settle_success({ bytes => $bytes });
    return 1;
}

sub _fake_complete_splice_error ($self, $token, %arg) {
    my ($rec, $op) = $self->_take_pending_and_unregister('pending_splices', $token) or return 0;

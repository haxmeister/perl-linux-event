...
use Linux::Event::Listener::Error;
use Linux::Event::Listener::Peer;
...
sub _descriptor_for ($class) {
    ...
    croak 'Linux::Event::Listen is a base class; construct a Listen subclass'
        if $class eq __PACKAGE__;
    croak "$class is not a Linux::Event::Listen subclass"
...
sub _setup_error (%arg) {
    die Linux::Event::Listener::Error->new(
        type      => 'setup',
        fatal     => 1,
        %arg,
    );
...
sub new ($class, %opt) {
    ...
    if ($fh_mode) {
        ...
        my $local = Linux::Event::Listener::Peer->new(getsockname($fh));
...
sub _accept_ready ($self) {
    ...
    while ($at < @$batch) {
        ...
        my $peer = Linux::Event::Listener::Peer->new($sockaddr);
        $self->{accepted}++;
...
sub _accept_error ($self, $errno) {
    ...
    my $error = Linux::Event::Listener::Error->new(
...
Linux::Event::Listen - native accepting listeners for Linux::Event
...
=head1 SYNOPSIS
  ...
  sub on_accept ($listener, $fh, $peer) {
      EchoStream->new(loop => $listener->loop, fh => $fh);
  }
...
Linux::Event::Listen is the compatibility generic-accept Watcher. It creates or
adopts a listening stream socket and drains accepted connections with native
C<accept4>. It transfers each nonblocking, close-on-exec client filehandle to a
cached subclass C<on_accept> method.

Listen does not construct Linux::Event::Stream or implement application
protocols. The accepted socket may be handed to Stream, a raw watcher, or
another protocol engine.

=head1 CONSTRUCTOR

=head2 new

The base class is not constructible. Concrete subclasses must define
C<on_accept> and C<on_error>.

Exactly one socket source is required:

  host => $host, port => $port

  unix => $path

  fh => $existing_listening_socket

TCP port zero is supported; C<port> then reports the assigned port. Host C<*>
uses the system passive wildcard. An adopted socket is made nonblocking and
close-on-exec and is not owned unless C<owns_socket =E<gt> 1> is supplied.

Options:

=over 4

=item * C<backlog> - default 4096

=item * C<max_accept_per_tick> - default 256; zero drains until retry

=item * C<edge_triggered> - default false; requires an unlimited drain

=item * C<reuseaddr> - default true for created TCP sockets

=item * C<reuseport> - default false

=item * C<v6only> - optional IPv6-only socket policy

=item * C<unlink> - safely remove an existing Unix socket before bind

=item * C<unlink_on_close> - default true for a created Unix socket

=item * C<permissions> - optional Unix socket filesystem mode

=item * C<data> - application state

=back

Setup failures throw a L<Linux::Event::Listener::Error> synchronously. Runtime
accept and listener failures call C<on_error>. The canonical error class
inherits C<Linux::Event::Listen::Error> for compatibility.

Construction is detached. C<< $loop->add($listener) >> starts readiness
observation. The former C<loop =E<gt> $loop> option remains compatibility
syntax. New Stream servers should normally use C<< MyStream->listen >>, which
returns L<Linux::Event::Listener> and removes accepted-filehandle plumbing.
...
Required. Ownership of C<$fh> transfers to the callback. C<$peer> is a lazy
L<Linux::Event::Listener::Peer> object; address text is formatted only when an
accessor is called. It inherits C<Linux::Event::Listen::Peer> for compatibility.

=head2 on_error

  sub on_error ($listener, $error) { ... }

Required. Resource exhaustion pauses read readiness before this callback to
avoid a busy loop. The application may release resources and call C<resume>.
Terminal listener errors close an owned socket and enter C<failed> state before
the callback.
...
Control accepted-connection readiness without replacing the watcher.

=head2 close / cancel

Stop accepting. An internally created or explicitly owned listening socket is
closed. A created Unix socket path is removed according to
C<unlink_on_close>.

=head2 detach

Stop watching and return the listening filehandle without closing it.

=head2 loop / fh / fd / data

Return core request values. C<data($new_value)> replaces application data.

=head2 host / port / path / family

Return the bound local address. Port reflects a kernel-assigned ephemeral port.

=head2 state / is_running / accepted / last_error

Report listener lifecycle and accepted-connection counts. State is initially
C<unattached>, becomes C<listening> after C<add>, and ends as C<closed>,
C<failed>, or C<detached>.

=head1 ACCEPT POLICY

Native C<accept4> requests C<SOCK_NONBLOCK | SOCK_CLOEXEC> atomically. The
default level-triggered watcher accepts at most 256 connections per dispatch,
preserving fairness while allowing epoll to report remaining backlog again.
Edge-triggered operation is permitted only with an unlimited drain so a
fairness cap cannot strand queued clients.

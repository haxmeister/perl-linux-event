...
my $json = "$dir/first-class-raw-callback.json";
my $script = "$Bin/../bench/run-first-class-raw-callback-bench.pl";
my @cmd = (
    $^X, '-Mblib', $script,
...
is($?, 0, 'first-class raw callback benchmark exits successfully')
...
close $fh;
is($report->{benchmark}, 'linux-event-first-class-raw-callback-dispatch',

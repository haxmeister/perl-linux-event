...
{
    package T::VarintSubclass;
    our @ISA = ('Linux::Event::Stream::Framer::Varint');
...
my $sub_stream = Linux::Event::Stream->new(
    loop => $sub_loop,
    fh => $sa,
    framer => T::VarintSubclass->new,
    on_message => sub ($s, $message) { $custom = $message; $sub_loop->stop },
);
is($sub_stream->{framing_backend}, 'xs-perl', 'Varint subclass stays on custom plug-in path');
syswrite($ca, 'custom!');
$sub_loop->run;
...
my $shared = Linux::Event::Stream::Framer->varint;
...
is($x->{framing_backend}, 'xs', 'shared Varint is native on Stream X');
is($y->{framing_backend}, 'xs', 'shared Varint is native on Stream Y');
syswrite($cx, "\x80");
syswrite($cy, $shared->frame('other'));
syswrite($cx, "\x01" . ('x' x 128));
$loop->run;
is($got{x}, 'x' x 128, 'Stream X retains its own partial prefix state');
